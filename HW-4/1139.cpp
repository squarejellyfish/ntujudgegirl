  ...
  public:
    ...
    void encrypt(char plaintext[])
    {
        if (check_valid_stream(plaintext))
        {
            ...
            {
                text[i] =
                    ((key[i % strlen(key)] - '0') ^ (plaintext[i] - '0')) + '0';
            }
            text[i] = '\0';
        ...
        {
            cout << "Plaintext contains character other than 0 and 1. ENCRYPT "
                    "FAILED.\n";
        }
    }
    // 把密文解密成明文
    void decrypt(char chiphertext[])
    {
        if (check_valid_stream(chiphertext))
        {
            ...
            {
                text[i] =
                    ((key[i % strlen(key)] - '0') ^ (chiphertext[i] - '0')) + '0';
            }
            text[i] = '\0';
        ...
        {
            cout << "Chiphertext contains character other than 0 and 1. "
                    "ENCRYPT FAILED.\n";
        }
    }
    // 更新key的内容
    void updatekey(char newkey[])
    {
        ...
        {
            cout << "NEW KEY contains character other than 0 and 1. UPDATE KEY "
                    "FAILED.\n";
        }
    }
    // 結束程式
    void ended() { cout << "MACHINE HALTING...\n"; }
};
